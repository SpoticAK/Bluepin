...
export function getAiClient(): GoogleGenAI {
  if (!aiClient) {
    ...
    if (!apiKey) {
      console.error(
        "[Gemini API] WARNING: GEMINI_API_KEY is not set in environment variables or .env file!",
      );
    }
    ...
    try {
      ...
      if (
        ...
        if (options.model === "gemini-3.5-flash-lite") {
          console.log(
            `[Gemini API] Falling back from ${options.model} to gemini-3.5-flash-lite`,
          );
        ...
        console.log(
          `[Gemini API] Rate limit or high demand (503/429). Retrying with model ${options.model} in ${Math.round(waitMs / 1000)}s... (Attempt ${i + 1}/${maxRetries})`,
        );
