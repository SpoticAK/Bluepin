...
export function WeightCard() {
  ...
  if (hasEntries && profile.heightCm) {
    ...
    if (bmiCategory === 'Normal weight') badgeColor = 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20';
    else if (bmiCategory === 'Underweight') badgeColor = 'bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20';
    else if (bmiCategory === 'Overweight') badgeColor = 'bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/20';
    else badgeColor = 'bg-red-500/10 text-red-600 dark:text-red-400 border-red-500/20';
  }
  ...
  const yDomain = [Math.max(0, Math.floor(minWeight - 5)), Math.ceil(maxWeight + 5)];
    ...
    <section className="bg-theme-card p-5 sm:p-6 rounded-[28px] border border-theme-border/50 shadow-sm flex flex-col gap-4">
      ...
      {hasEntries ? (
        <div className="flex flex-col gap-4">
          <div className="flex items-start justify-between">
            <div className="flex flex-col">
              <span className="text-[14px] font-semibold text-theme-text-sec mb-1">Latest Weight</span>
              <div className="flex items-baseline gap-1.5">
                <span className="text-4xl font-display font-medium text-theme-text tracking-tight">{latestEntry.weight}</span>
                <span className="text-sm font-bold text-theme-text-sec">kg</span>
              </div>
              <span className="text-[11px] font-medium text-theme-text-sec mt-1">Logged {safeFormat(latestEntry.date, 'MMM d, yyyy')}</span>
            </div>
            
            {profile.heightCm ? (
              <div className="flex flex-col items-end text-right">
                <span className="text-[14px] font-semibold text-theme-text-sec mb-1">BMI</span>
                <span className="text-2xl font-display font-medium text-theme-text">{bmi}</span>
                <div className={cn("mt-1.5 px-2.5 py-0.5 rounded-full border text-[11px] font-bold whitespace-nowrap", badgeColor)}>
                  {bmiCategory}
                </div>
              </div>
            ) : (
              <div className="flex flex-col items-end text-right">
                <span className="text-[14px] font-semibold text-theme-text-sec mb-1">BMI</span>
                <span className="text-sm text-theme-text-sec">Height required</span>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center gap-2 mt-2">
            {(['30d', '90d', 'all'] as const).map(filter => (
              ...
              >
                {filter === '30d' ? '30 Days' : filter === '90d' ? '90 Days' : 'Lifetime'}
              </button>
            ))}
          </div>
          ...
          {filteredEntries.length > 0 && (
            <div className="h-40 w-full mt-2 -ml-3">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={filteredEntries} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  <defs>
                  ...
                  <YAxis 
                    ...
                    axisLine={false}
                    tickLine={false}
                    tick={{ fontSize: 10, fill: 'var(--theme-text-sec)' }}
                    tickCount={5}
                  />
                  <XAxis 
                    dataKey="date" 
          ...
          <div className="mt-2 border-t border-theme-border/50 pt-4">
            ...
            {isExpanded && (
              <div className="mt-4 flex flex-col gap-3 animate-in slide-in-from-top-2 fade-in duration-200">
                {recentLogs.map((log) => (
                  <div key={log.id} className="flex justify-between items-center text-[14px] py-1">
        ...
        <div className="py-6 flex flex-col items-center justify-center text-center">
          <p className="text-[14px] font-medium text-theme-text">No weight logged</p>
          <p className="text-[12px] text-theme-text-sec mt-1 max-w-[200px]">Start tracking your weight to calculate BMI and see your trends.</p>
        </div>
      )}
      ...
      {showAllLogs && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/20 backdrop-blur-sm animate-in fade-in" onClick={() => setShowAllLogs(false)}>
          <div className="bg-theme-card rounded-[24px] w-full max-w-sm shadow-xl overflow-hidden animate-in zoom-in-95 flex flex-col max-h-[80vh]" onClick={e => e.stopPropagation()}>
            <div className="p-4 border-b border-theme-border flex justify-between items-center bg-theme-bg shrink-0">
              ...
              <button onClick={() => setShowAllLogs(false)} className="text-theme-text-sec hover:text-theme-text">
                ✕
              </button>
            </div>
            <div className="overflow-y-auto p-4 flex flex-col gap-3">
              {reverseSortedEntries.map((log) => (
                <div key={log.id} className="flex justify-between items-center text-[14px] border-b border-theme-border/30 pb-3 last:border-0 last:pb-0">
