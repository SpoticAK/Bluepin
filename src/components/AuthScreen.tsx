...
export default function AuthScreen() {
 ...
 />
            ...
            {showConsentError && !isLogin && !canSignUp && (<p className="text-xs text-theme-critical font-medium mb-3">* Please give the required consents above.</p>)}
  ...
  <button 
    type="button"
    onClick={handleGoogleAuth} disabled={loading || googleLoading}
  ...
  <p className="text-[11px] text-theme-text-sec text-center leading-relaxed px-1 mb-6">
    ...
    <button type="button" onClick={() => setOpenLegalDoc('terms')} className="text-theme-accent hover:underline font-medium">Terms of Service</button>
    {' '}and{' '}
    <button type="button" onClick={() => setOpenLegalDoc('privacy')} className="text-theme-accent hover:underline font-medium">Privacy Policy</button>,
    {' '}and consent to personal and health data processing.
  </p>

 <p className="text-center text-sm text-theme-text-sec">
 {isLogin ? "Don't have an account? " : "Already have an account? "}
 <button type="button" onClick={handleToggleMode} className="font-bold text-theme-text hover:underline">
